...
import * as imgpick from "expo-image-picker"; // To choose photo from Gallery or Click form Camera.
...
import ImgPicker from "../../components/imgpick"; // The code for the Image component will be shown in the video below
import * as UserActions from "../../store/actions/AllUsers"; // Our all Redux User actions
import { storage } from "../../firebase/FirebaseConf"; //  Firebase conf file will be shown in the video
...
export default fxn AddDetails({ navigation }) {
  ...
  const DispatchAction = async (name, image, desc) => {
    // This will dispatch Our action
    try {
      await dispatch(UserActions.Create_User(name, image, desc));
  ...
  const TakePhotoFromCamera = async () => {
    ...
    const Imageresult = await imgpick.launchCameraAsync({
      allowsEditing: true,
      aspect: [16, 9],
      quality: 0.5,
    });
  ...
  const ChooseFromGallery = async () => {
    ...
    let result = await imgpick.launchImageLibraryAsync({
      mediaTypes: imgpick.MediaTypeOptions.All,
      allowsEditing: true,
      aspect: [4, 3],
      quality: 1,
    });
